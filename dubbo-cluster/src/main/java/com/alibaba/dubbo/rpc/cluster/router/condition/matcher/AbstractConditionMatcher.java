 ...
 */
package com.alibaba.dubbo.rpc.cluster.router.condition.matcher;
...
import static com.alibaba.dubbo.common.Constants.METHODS_KEY;
import static com.alibaba.dubbo.common.Constants.METHOD_KEY;
...
public abstract class AbstractConditionMatcher implements ConditionMatcher {
    ...
    public AbstractConditionMatcher(String key) {
        this.key = key;
        this.valueMatchers = ExtensionLoader.getExtensionLoader(ValuePattern.class).getActivateExtensions();
    }
    ...
    protected boolean doPatternMatch(String pattern, String value, URL url, Invocation invocation, boolean isWhenCondition) {
        ...
        logger.error("Executing condition rule value match expression error. Match pattern is " + pattern + ", value is " + value + ", condition type " + (isWhenCondition ? "when" : "then"));

        ValuePattern paramValueMatcher = ExtensionLoader.getExtensionLoader(ValuePattern.class).getExtension("wildcard");
