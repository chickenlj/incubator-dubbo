 ...
 */
package com.alibaba.dubbo.rpc.cluster.router.condition.matcher.pattern.range;
...
public class RangeValuePattern implements ValuePattern {
    private static final Logger logger = LoggerFactory.getLogger(RangeValuePattern.class);
    ...
    public boolean match(String pattern, String value, URL url, Invocation invocation, boolean isWhenCondition) {
        ...
        try {
            ...
            if (arr.length < 2) {
                logger.error("Invalid condition rule " + pattern + " or value " + value + ", will ignore.");
                return defaultValue;
            }
            ...
            logger.error("Parse integer error, invalid condition rule " + pattern + " or value " + value + ", will ignore.", e);
            return defaultValue;
        }
